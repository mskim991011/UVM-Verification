...
class ram_scoreboard extends uvm_scoreboard;
    ...
    virtual function void write(ram_seq_item item);
        ...
        if (item.wr) begin
            ...
            `uvm_info(get_type_name(), $sformatf("addr = 0x%02h data = 0x%04h",item.addr,item.wdata), UVM_MEDIUM);
        end else begin
            expected = ref_mem[item.addr];
            if (item.rdata === expected) begin
                pass_cnt++;
                `uvm_info(get_type_name(), $sformatf("PASS: addr = 0x%02h expected = 0x%04h real data = 0x%04h",item.addr,expected,item.rdata), UVM_MEDIUM);
            end else begin
                fail_cnt++;
                `uvm_error(get_type_name(), $sformatf("FAIL: addr = 0x%02h expected = 0x%04h real data = 0x%04h",item.addr,expected,item.rdata));
            end
        end
    endfunction
