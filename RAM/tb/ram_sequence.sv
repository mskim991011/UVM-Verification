...
class ram_all_write_read_sequence extends uvm_sequence#(ram_seq_item);
    `uvm_object_utils(ram_all_write_read_sequence)
    int num_transaction = 0;

    function new(string name = "ram_all_write_read_sequence");
        super.new(name);
    endfunction
    ...
    virtual task body();
            ...
            for (int i = 0; i<2**8; i++)begin
                ...
                start_item(item);
                if(!item.randomize() with {wr ==1; addr ==i;}) `uvm_fatal(get_type_name(), "Randomization Fail!!!");
                finish_item(item);
            end
