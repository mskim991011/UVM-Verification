...
interface uart_if(input logic clk, input logic reset);
    ...
    clocking drv_cb @(posedge clk);
        ...
        input  tx_busy, rx_valid, rx_data;
    endclocking

    clocking mon_cb @(posedge clk);
        default input #1ns output #1ns;
        input tx_data, tx_start, tx_busy, tx, rx, rx_data, rx_valid;
    endclocking
endinterface
...
class uart_scoreboard extends uvm_scoreboard;
    ...
    function void write(uart_seq_item item);
        if (item.tx_data !== item.rx_data) begin
            fail_cnt++;
            `uvm_error(get_type_name(), $sformatf("Mismatch!! tx_data = 0x%2h rx_data = 0x%2h", item.tx_data, item.rx_data));
        end else begin
            pass_cnt++;
            `uvm_info(get_type_name(), $sformatf("MATCH Success!! tx_data = 0x%2h rx_data = 0x%2h", item.tx_data, item.rx_data), UVM_MEDIUM);
        end
    endfunction
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        if (fail_cnt > 0) begin
            ...
            `uvm_info(get_type_name(), $sformatf("TEST PASSED!!!! %0d complete", pass_cnt), UVM_LOW);
        end
        `uvm_info(get_type_name(), "\n\n", UVM_LOW);
    endfunction
endclass
...
class uart_monitor extends uvm_monitor;
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(virtual uart_if)::get(this,"","u_if",u_if)) begin
            `uvm_fatal(get_type_name(), "cannot find uart_if in config_db");
        end
    endfunction
...
class uart_driver extends uvm_driver #(uart_seq_item);
    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            @(u_if.drv_cb);
            u_if.tx_start <=1'b0;
            `uvm_info(get_type_name(), $sformatf("Send complete: tx_data = 0x%02h", item.tx_data), UVM_HIGH);
            @(u_if.drv_cb);
            while (!u_if.drv_cb.tx_busy) @(u_if.drv_cb);
            while (u_if.drv_cb.tx_busy) @(u_if.drv_cb);
            `uvm_info(get_type_name(), $sformatf("Send complete: tx_data = 0x%02h", item.tx_data), UVM_HIGH);
...
module tb_uart_uvm();
    ...
    uart #(.SYS_CLK(100_000_000), .BAUD_RATE(9600)) dut (
        .clk(clk),
        .reset(reset),
